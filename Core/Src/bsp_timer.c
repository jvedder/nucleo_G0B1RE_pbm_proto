...
/* Set TIM clock rate = 64MHz / 16 = 4MHz */
/* Each TIM tick is 0.25 uS */
#define TIM_PRESCALER (16u)

/* TIM Period is 1 mS at 4 MHz (Caution:16-bit value: 65K max) */
...
#else
/* all strobes the same to measure timer to timer delay */
/* Near IR */
#define TIM_STROBE1_ON  (500u)
#define TIM_STROBE2_ON  (500u)
#define TIM_STROBE3_ON  (500u)
#define TIM_STROBE4_ON  (500u)
/* Red */
#define TIM_STROBE5_ON  (500u)
#define TIM_STROBE6_ON  (500u)
#define TIM_STROBE7_ON  (500u)
#define TIM_STROBE8_ON  (500u)
#endif

/* IR Pulse width is 80 uS per 1 mS (2.5%) */
#define TIM_IR_PULSE_WIDTH (320u)
...
#else
    /* Timers are independent. Start them back-to-back as quickly as possible */
    /* Approximate measured timing skew on oscilloscope: */
    /*     from TIM1 -> TIM2 is   80 ns. */
    /*     from TIM1 -> TIM3 is  210 ns. */
    /*     from TIM1 -> TIM4 is  330 ns. */
    /*     from TIM1 -> TIM15 is 440 ns. */
    __disable_irq();
    TIM1->CR1 = TIM_CR1_CEN;
    TIM2->CR1 = TIM_CR1_CEN;
    TIM3->CR1 = TIM_CR1_CEN;
